...
def gen_headers(usercookie, host=config.HOST_TIEBA):
    data = {'Cookie': usercookie, 'Host': host}
...
def get_sign_data(bduss, fid, tbs, barname):
    ...
    data = (
        ('BDUSS', bduss),
        ('_client_id',
            '04-00-DA-69-15-00-73-97-08-00-02-00-06-00-'
            '3C-43-01-00-34-F4-22-00-BC-35-19-01-5E-46'),
        ('_client_type', '4)'),
...
def http(method, url, headers=config.headers, **kwargs):
...
def parsing_print(username, barname, data):
    ...
    printf('%s@tieba %s 成功簽到, exp +%s, '
           '本吧第%s個簽到: 連續簽到%s天/累計簽到%s天 - 本月漏簽%s天' %
